...
allowedTypes = set('pedestrian,living_street,bridleway,road,trunk,unclassified,path,steps,primary,secondary,tertiary,service,residential'.split(','))
...
schema = {'geometry': 'LineString', 'properties': {'name': 'str', 'length': 'int', 'azimuth_d': 'float'}}
sun_az = 0.0
    ...
    with fiona.open(outp, 'w', crs=source.crs, driver=source.driver, schema=schema) as target:
        for g in source:
            ...
            for s in segments:
                ...
                shape = {'geometry': geom, 'type': 'Feature', 'properties': {'name': name, 'length': l, 'azimuth_d':az_diff}}
                target.write(shape)
...
print "Total length: %d, solstice length: %d, expected value: %d, anomaly: %d%%, sun azimuth: %f" %(tl, sl, 6*tl/180, 100*sl/(6*tl/180), sun_az)

stats = {'totalLength': tl, 'solsticeLength': sl,'sun_azimuth': sun_az, 'ratio': int(10000*sl/tl), 'hist':h}
